...
const SAMPLE_PROJECT_URL = "https://chat-companion-app-68.lovable.app"
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  // Verify Supabase auth-hook signature (Standard Webhooks).
  let payload: any
  try {
  ...
  const confirmationUrl = `${ed.site_url}/auth/v1/verify?token=${ed.token_hash}&type=${ed.email_action_type}&redirect_to=${encodeURIComponent(ed.redirect_to || '')}`
  const templateProps = {
    siteName: SITE_NAME,
